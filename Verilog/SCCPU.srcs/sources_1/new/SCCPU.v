...
input reset
    ...
    wire [31:0] nextPC;
    wire [31:0] inst;
    wire fetch_enable = buffer_empty && !(stall | stallCU) && !halted;

    nextPC pcgen(.PC((branching || jump)? IF_ID_PC : PC), .imm(immediate), .EX_MEM_Imm(EX_MEM_Imm), .EX_MEM_PC(EX_MEM_PC), .rs1(data1), .stall(stall | stallCU), .branching(branching), .match(match), .branchF(branch), .jumpF(jump), .JALR(JALR), .nextPC(nextPC));
...
//flushing needs to be tested , stalling needs to be tested , the unified memory parameters are changed to smaller values to test the loads and stores 
    ...
    UnifiedMem Memory(
        .clk(clk),
        .address(fetch_enable ? PC : EX_MEM_ALU_out)  ,
         .MemRead(fetch_enable ? 2'b11 : EX_MEM_Ctrl_MEM[5:4]),
        .MemWrite(fetch_enable ? 2'b00 :EX_MEM_Ctrl_MEM[3:2]),
        .writedata(EX_MEM_midmux), //This is a hardwire value for testibf
        .read_data(read_data),
...
Buffer buffer_inst(
    ...
    nBitReg #(64) IF_ID (
        .clk(clk),
        .rst(reset),
        .load(IF_ID_enable), //hardwired value , needs to be changes to IF/ID enable or the fetch enable from above (under testing) 
        .D({PC, next_IF_inst}),    
    ...
    .rst(reset), .clk(clk));
     ...
     nBitReg #(196) ID_EX (clk,reset,1'b1,
                           ((stall || ~match)? 196'b0 : {ctrl_EX, ctrl_MEM, ctrl_WB,
    ...
    //Excute stage
    ALUControlUnit alucu (.instruction(ID_EX_Func), .ALUop(ID_EX_Ctrl_EX[1:0]), .clk(clk), .ALUSELECT(ALUSELECT), .fullinstruction(ID_EX_Inst) );
    

    assign shamt =
        ((opcode == `OPCODE_Arith_I) && ((funct3 == `F3_SLL) || (funct3 == `F3_SRL))) ? ID_EX_Inst[`IR_shamt] :
        ((opcode == `OPCODE_Arith_R) && ((funct3 == `F3_SLL) || (funct3 == `F3_SRL))) ? midmux[4:0] :
        5'b00000;
    ...
    n4x1MUX ALU1 (.a(ID_EX_data1), .b(writedata), .c(EX_MEM_ALU_out), .d(ID_EX_PC), .s(ID_EX_Ctrl_EX[2]? 2'b11 : forA), .out(alu1));
    n4x1MUX ALU2 (.a(ID_EX_data2), .b(writedata), .c(EX_MEM_ALU_out), .d(32'b0), .s(forB), .out(midmux));
    ...
    nBitReg #(149) EX_MEM (
    clk, reset, 1'b1,
    (~match? 149'b0 : {ID_EX_Ctrl_MEM, ID_EX_Ctrl_WB, ID_EX_PC, ID_EX_Imm, alures, ID_EX_Rd, ID_EX_BF3, {zero, cf, vf, sf} , midmux}),
    
    {EX_MEM_Ctrl_MEM, EX_MEM_Ctrl_WB,  EX_MEM_PC, EX_MEM_Imm, EX_MEM_ALU_out, EX_MEM_Rd, EX_MEM_BF3, EX_MEM_ALUF , EX_MEM_midmux}
    );
...
//    DataMem datamemory (.clk(clk), .MemRead(MemRead), .MemWrite(MemWrite), .addr(alures[7:2]), .data_in(data2), .data_out(memout));
    ...
    nBitReg #(104) MEM_WB (clk,reset,1'b1,
                            {EX_MEM_Ctrl_WB, read_data[31:0], EX_MEM_ALU_out, EX_MEM_Rd, EX_MEM_PC},
